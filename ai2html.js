...
function main() {
  ...
  function matchFont(aifont) {
    ...
    var fontFamilies = {
      ...
      "source": '"Source Sans Pro", -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Helvetica, Arial, sans-serif',
    ...
    var findWeight = function() {
      var weight = aifont.match(/(semi-?bold|bold|black|extra?-light|light|regular|medium|ultra-?light)/gi);
    ...
    var findStyle = function() {
      ...
      var style = aifont.match(/(italic|(?:semi-?bold|bold|black|extra?-light|light|regular)it)/gi);
      if(style) {
        return "italic";
      }
      return "";
  ...
  function generateMediaQueries() {
    ...
    function queryCss(breakpoint, css) {
      if(breakpoint) {
    ...
    forEach(sizes, function(size, i) {
      ...
      // remove the current board from the array and generate css to show it.
      var toShow = show(selectBoard(temp.splice(i, 1)[0].name));
